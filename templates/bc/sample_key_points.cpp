...
void add(int u, int v, int mode=0) {
  Edge e;
  e.u = u; e.v = v;
  float dx, dy, dz;
  if (mode==0)
  {
    dx = surface.V(u, 0) - surface.V(v, 0);
    dy = surface.V(u, 1) - surface.V(v, 1);
    dz = surface.V(u, 2) - surface.V(v, 2);
  }
  else
  {
    dx = 0.1;
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < m; i++) {
    ...
    add(a, b, mode); add(b,a, mode); add(a,c, mode); add(c,a, mode); add(b,c, mode); add(c,b, mode);
  }
